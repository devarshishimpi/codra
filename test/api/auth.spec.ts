...
describe("Dashboard API: auth, session and account", () => {
  ...
  it("starts GitHub OAuth with the configured callback and scope", async () => {
    ...
    expect(url.origin + url.pathname).toBe(
      "https://fake-provider.com/authorize",
    );
  ...
  it("rejects GitHub users outside the allowlist", async () => {
    ...
    const authStart = await app.request("/auth/github", {}, env);
    const state = new URL(authStart.headers.get("location")!).searchParams.get(
      "state",
    );
  ...
  it("rejects authenticated state-changing API requests without the CSRF header", async () => {
    ...
    const response = await app.request(
      "/api/jobs/non-existent-id/retry",
      {
        method: "POST",
        headers: { Cookie: `codra_session=${token}` },
      },
  ...
  it("preserves omitted review settings when patching a single setting", async () => {
    ...
    try {
      ...
      const commentsOnly = await app.request(
        "/api/settings",
        {
          method: "PATCH",
          headers,
      ...
      const afterCommentsOnly = await app.request(
        "/api/settings",
        {
          headers: { Cookie: `codra_session=${token}` },
        },
      ...
      const concurrencyOnly = await app.request(
        "/api/settings",
        {
          method: "PATCH",
          headers,
      ...
      const afterConcurrencyOnly = await app.request(
        "/api/settings",
        {
          headers: { Cookie: `codra_session=${token}` },
        },
  ...
  it("falls back invalid stored review settings independently", async () => {
    ...
    try {
      ...
      const invalidConcurrency = await app.request(
        "/api/settings",
        {
          headers: { Cookie: `codra_session=${token}` },
        },
      ...
      const invalidComments = await app.request(
        "/api/settings",
        {
          headers: { Cookie: `codra_session=${token}` },
        },
  ...
  it.each([
    [
      "updates the editable account display name",
      { name: "Renamed Codra User" },
      200,
      (data: AccountResponse) =>
        expect(data.account.accountName).toBe("Renamed Codra User"),
    ],
    [
      "persists a display time zone",
      { timezone: "Asia/Kolkata" },
      200,
      (data: AccountResponse) =>
        expect(data.account.timezone).toBe("Asia/Kolkata"),
    ],
    [
      "clears display time zone back to default",
      { timezone: null },
      200,
      (data: AccountResponse) => expect(data.account.timezone).toBeNull(),
    ],
    [
  ...
  it("keeps a user-set display name when the OAuth upsert runs again", async () => {
    ...
    await app.request(
      "/api/auth/account",
      {
        method: "PATCH",
        headers,
        body: JSON.stringify({ name: "My Chosen Name" }),
      },
      env,
    );
    ...
    expect(((await read.json()) as AccountResponse).account.accountName).toBe(
      "My Chosen Name",
    );
  });
  ...
  it("syncs an updates email only once per GitHub user", async () => {
    ...
    await expect(
      syncUpdatesEmail(env.APP_KV, 42, "user@example.com"),
    ).resolves.toBe(true);
    await expect(
      syncUpdatesEmail(env.APP_KV, 42, "user@example.com"),
    ).resolves.toBe(false);
  ...
  it("clamps an out-of-range stored value instead of falling back to the default", async () => {
    await runWithDb(env, async () => {
      ...
      try {
        ...
        expect((await getReviewSettings(env)).maxFiles).toBe(
          reviewMaxFilesRange.max,
        );
