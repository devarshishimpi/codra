...
import {
  ...
  getLlmProvider,
} from "@codraoss/db/model-configs";
import {
  encryptLlmApiKey,
  decryptLlmApiKey,
  listProviderModels,
...
export function createSharedApiDeps(p: PlatformDeps): ApiRouterDeps {
  return {
    ...
    gitProvider: {
      getAppInstallationUrl: async () =>
        await GitHubClient.getAppInstallationUrl(toAppBindingsConfig(p)),
      listInstallations: async () =>
        await GitHubClient.listInstallations(toAppBindingsConfig(p)),
      createService: (installationId?: number | string | null) =>
        new GitHubClient(
          toAppBindingsConfig(p),
          installationId != null ? String(installationId) : undefined,
        ),
    },
    config: {
      getGlobalConfig: async () => await getGlobalConfig(p.kv),
      updateGlobalConfig: async (config: any) =>
        await updateGlobalConfig(p.kv, config),
      loadRepoConfig: async (input: any) =>
        await loadRepoConfig(p.kv, p.db, input),
      invalidateRepoConfigCache: async (owner: string, repo: string) =>
        await invalidateRepoConfigCache(p.kv, owner, repo),
    },
    modelRunner: {
              ...
              syncErrors.push({
                ...
                error:
                  error instanceof Error
                    ? error.message
                    : "Could not refresh provider models.",
              });
            }
          }),
        );
      ...
      testConnection: async (modelId: string) => {
        ...
        try {
          const input = {
            systemPrompt:
              "You are validating connectivity. Return only the JSON object.",
            userPrompt:
              'Return an empty review: no findings, overall_correctness "patch is correct".',
            responseSchema: buildReviewResponseSchema(1),
          };
          let response;
          if (config.apiFormat === "cloudflare-workers-ai") {
            response = await reviewWithCloudflare(
              p.aiBinding,
              config.modelName,
              input,
              undefined,
            ...
            if (!config.encryptedApiKey) {
              throw {
                ...
                message: `Provider ${config.providerName} does not have a saved API key.`,
              };
            }
            ...
            switch (config.apiFormat) {
              case "gemini":
                response = await reviewWithGoogle(
                  {
                    apiKey,
                ...
                break;
              case "vertex":
                response = await reviewWithVertex(
                  {
                    apiKey,
                    baseUrl: config.baseUrl,
                    providerName: config.providerName,
              ...
              case "openai":
                ...
                break;
              case "anthropic":
                response = await reviewWithAnthropic(
                  {
                    apiKey,
                    baseUrl: config.baseUrl,
                    providerName: config.providerName,
          ...
          return {
            ...
            ...(response.degraded === "schema-dropped"
              ? {
                  ...
                  warning:
                    "Connected, but this endpoint rejected the response grammar. Reviews will run without constrained decoding.",
                }
              : {}),
          };
        } catch (error) {
        ...
        try {
          encryptedApiKey =
            input.apiFormat === "cloudflare-workers-ai"
              ? null
              : input.apiKey
                ? await encryptLlmApiKey(getSecretStore(p), input.apiKey.trim())
                : null;
        } catch (error) {
          if (
            error instanceof Error &&
            error.message.includes("LLM_CONFIG_ENCRYPTION_KEY")
          ) {
      ...
      updateProviderWithSecret: async (id: string, input: any) => {
        ...
        try {
          if (input.apiFormat === "cloudflare-workers-ai") {
            ...
            encryptedApiKey = await encryptLlmApiKey(
              getSecretStore(p),
              input.apiKey.trim(),
            );
          } else {
        ...
        const effectiveEncryptedApiKey =
          encryptedApiKey !== undefined
            ? encryptedApiKey
    ...
    platform: {
      ...
      getUpdatesEmailPreference: async (githubUserId: number) =>
        await getUpdatesEmailPreference(p.kv, githubUserId),
      syncUpdatesEmail: async (
        githubUserId: number,
        email: string | null | undefined,
      ) => await syncUpdatesEmail(p.kv, githubUserId, email),
    ...
    authProvider: {
      ...
      consumeOAuthState: async (state: string) =>
        await consumeOAuthState(p.kv, state),
      beginAuthorization: async (callbackUrl: string, state: string) =>
        await identityProvider(p).beginAuthorization(
          callbackUrl,
          state,
          toAppBindingsConfig(p) as any,
        ),
      completeAuthorization: async (
        ...
        await identityProvider(p).completeAuthorization(
          code,
          state,
          expectedState,
          toAppBindingsConfig(p) as any,
    ...
    webhook: {
      verifySignature: async (signature: string | null, body: string) =>
        await verifyGitHubWebhookSignature(
          p.githubAppWebhookSecret,
          signature,
          body,
        ),
      normalizePayload: (eventName: string, payload: any) =>
        normalizeGitHubWebhook(eventName, payload),
