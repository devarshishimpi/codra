...
const FINDING_MARKER_PATTERN =
  /<!--\s*codra-fp:([0-9a-f]+):([0-9a-f]*)(?::([0-9a-f]*))?\s*-->/;
...
export function formatFindingMarker(
  comment: Pick<
    ParsedReviewComment,
    "fingerprint" | "anchorHash" | "fingerprintV2"
  >,
) {
...
export function parseFindingMarker(body: string | null | undefined) {
  ...
  return {
    fingerprint: match[1],
    anchorHash: match[2] || null,
    fingerprintV2: match[3] || null,
  };
}
...
export class FormatterService {
  ...
  stripLeadingTags(text: string): string {
    ...
    while (current !== prev) {
      ...
      current = current
        .replace(
          /^([\u{1F300}-\u{1F9FF}\u{2600}-\u{27BF}\u{FE00}-\u{FEFF}]|\[QUALITY\]|\[SECURITY\]|\[BUG\]|\[P[0-3]\]|\[NIT\]|QUALITY|SECURITY|BUG|P[0-3]|NIT|[:\-\s\uFE0F]|[^\w\s])+/giu,
          "",
        )
        .trim();
    }
  ...
  formatReviewOverview(input: ReviewOverviewInput) {
    ...
    if (input.filesFailed > 0) {
      notes.push(
        `${plural(input.filesFailed, "file")} could not be reviewed, so this pass is incomplete.`,
      );
    }
    if (postedFindings === 0 && input.withheldFindings > 0) {
      ...
      notes.push(
        `${plural(input.withheldFindings, "candidate")} did not survive the evidence and claim gates — see the [dashboard](${this.baseUrl}) for what was dropped and why.`,
      );
    }
