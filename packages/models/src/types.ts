...
import type {
  ModelResponse as ModelResponseShape,
  ModelResponseSchema,
...
export class UnparseableModelResponseError extends Error {
  constructor(
    ...
    super(
      `Model ${model} produced no reviewable output (${reason}); the file review failed.`,
    );
    this.name = "UnparseableModelResponseError";
  }
}

export function attachPartialResponse(
  error: object,
  response: ModelResponseShape,
) {
  Object.defineProperty(error, "partialResponse", {
    value: response,
    configurable: true,
  });
}
...
function errorDetailText(error: unknown): string {
  ...
  for (const entry of details) {
    ...
    for (const candidate of [
      e,
      ...(Array.isArray(e.fieldViolations) ? e.fieldViolations : []),
    ]) {
      const description = (candidate as Record<string, unknown> | null)
        ?.description;
