...
export async function reviewFile(
  ...
  const results: Array<
    ModelResponse & {
      parsed: ReturnType<typeof parseFileReviewResponse>;
  ...
  for (const [chunkIndex, chunk] of chunks.entries()) {
    ...
    if (results.length > 0 && ctx.tracker?.isNearLimit()) {
      logger.warn(
        `Stopping chunk processing for ${filePath} early due to subrequest budget limits.`,
      );
      break;
    }
    ...
    if (chunkIndex >= BASE_CHUNKS) {
      const remaining =
        ctx.tracker?.remainingSafeBudget() ?? Number.POSITIVE_INFINITY;
      if (remaining < EXTRA_CHUNK_BUDGET_RESERVE) {
        logger.info(
          `Skipping the opportunistic chunk tail for ${filePath}; budget is committed elsewhere.`,
          {
            chunkIndex,
    ...
    try {
      ...
      logger.warn(
        `Chunk review failed for ${filePath}, returning partial results to avoid stalling the job.`,
  ...
  const primaryResult =
    results.find((r) => r.parsed.verdict === "comment") ??
  ...
  return {
    ...
    parsed: {
      ...
      evidenceStats: results.reduce(
        (acc, r) => ({
          ...
          contextOnly:
            acc.contextOnly + (r.parsed.evidenceStats?.contextOnly ?? 0),
        }),
        {
          total: 0,
          matched: 0,
      ...
      claimTypeCounts: mergeCounts(
        results.map((r) => r.parsed.claimTypeCounts),
      ),
      deniedClaimCounts: mergeCounts(
        results.map((r) => r.parsed.deniedClaimCounts),
      ),
      absenceCheckStats: results.reduce(
        (acc, r) => ({
          absenceShaped:
            acc.absenceShaped +
            (r.parsed.absenceCheckStats?.absenceShaped ?? 0),
          identifierExtracted:
            acc.identifierExtracted +
            (r.parsed.absenceCheckStats?.identifierExtracted ?? 0),
    ...
    wasPromptTruncated:
      chunks.length < totalChunkCount || results.length < chunks.length,
...
async function reviewFileChunk(
  ...
  const response = await runModelChain(ctx, {
    systemPrompt,
    userPrompt,
    responseSchema: buildReviewResponseSchema(
      reviewBreadth(params.config.review),
    ),
    timeoutMs: adaptiveModelTimeoutMs(
      params.file.lineCount,
      outputBudgetTokens,
    ),
    outputBudgetTokens,
    ...
    parse: (rawText, { isLastModel }) => {
      ...
      if (
        ...
        logger.warn(
          "Model returned a non-answer for a substantive diff; escalating to the next model",
          {
...
export async function reviewFiles(
  ...
  const binLineCount = params.files.reduce(
  ...
  const response = await runModelChain(ctx, {
    systemPrompt,
    userPrompt,
    responseSchema: buildBatchReviewResponseSchema(
      reviewBreadth(params.config.review),
      params.files.length,
    ),
    ...
    config: params.config,
    parse: (rawText) =>
      parseBatchReviewResponse(rawText, params.files, {
        deniedClaimTypes: params.config.review.deny_claim_types,
