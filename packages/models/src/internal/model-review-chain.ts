...
import {
  ...
  SUBREQUEST_HEADROOM_FOR_MODEL_CALL,
} from "../limits";
import {
...
export async function runModelChain<T>(
  ...
  const {
    ...
    label,
    outputBudgetTokens,
    truncationIntolerant,
  } = params;
  const progressLabels = params.progressLabels?.length
  ...
  const recorded = await Promise.all(
    progressLabels.map((key) => ctx.chainProgress.startIndexFor(key)),
  );
  const startIndex = Math.min(
    Math.min(...recorded),
    Math.max(wholeChain.length - 1, 0),
  );
  ...
  let gateWaitMs = 0;
  const recordGateWait = (waitedMs: number) => {
    gateWaitMs += waitedMs;
  };
  for (const [modelIndex, currentModel] of modelsToTry.entries()) {
    ...
    if (modelIndex > 0 && ctx.tracker?.isNearLimit()) {
      logger.warn(
        `Skipping remaining fallback models for ${label}; subrequest budget for this invocation is nearly exhausted`,
        {
      ...
      if (sawTransientFailure) {
        lastTransientError =
          ...
          lastError ??
          new Error(
            "Per-invocation request budget was nearly exhausted before trying all configured fallback models",
          );
      }
      break;
    }
    ...
    const attemptTimeoutMs = chainAttemptTimeoutMs({
      ...
      remainingChainMs:
        MODEL_FALLBACK_CHAIN_BUDGET_MS -
        (Date.now() - chainStartedAt - gateWaitMs),
      hasAnotherModel: modelIndex < modelsToTry.length - 1,
    });

    if (modelIndex > 0 && attemptTimeoutMs === 0) {
      logger.warn(
        `Deferring ${label}: no room in the per-invocation time budget for another model`,
        {
      ...
      lastTransientError =
        ...
        lastError ??
        new Error(
          `Model fallback chain for ${label} exceeded its time budget; deferring for retry.`,
        );
      break;
    }
    ...
    if (
      resolved.apiFormat === "cloudflare-workers-ai" &&
      (await ctx.isProviderUnavailable(resolved.providerId))
    ) {
      logger.warn(
        `Skipping ${resolved.providerName} model ${currentModel} because the provider is unavailable for job ${ctx.jobId ?? "unknown"}`,
      );
      continue;
    }
    ...
    if (timingOut) {
      ...
      logger.info(
        `Skipping ${currentModel} for ${label}: it has repeatedly timed out on this job`,
        {
          isLastCandidate,
        },
      );
    ...
    if (
      ctx.tracker &&
      !ctx.tracker.hasRemainingSubrequests(SUBREQUEST_HEADROOM_FOR_MODEL_CALL)
    ) {
      logger.warn(
        `Deferring ${label}: not enough subrequest budget left to commit a prompt`,
        {
      ...
      lastTransientError =
        ...
        lastError ??
        new Error(
          `Per-invocation request budget was too low to attempt a model for ${label}; deferring for retry.`,
        );
      break;
    }

    const skipReason = await ctx.rateLimits.skipReason(
      resolved.modelName,
    ...
    try {
      ...
      await Promise.all(
        progressLabels.map((key) => ctx.chainProgress.clear(key)),
      );
      ...
      if (partial) {
        try {
          ...
          logger.warn(
            `Salvaged a truncated response from the last model in the chain for ${label}`,
            {
      ...
      if (
        resolved.apiFormat === "cloudflare-workers-ai" &&
        isCloudflareAllocationError(error)
      ) {
        await ctx.markProviderUnavailable(
          resolved.providerId,
      ...
      if (isSubrequestBudgetMessage(error)) {
        logger.warn(
          `Aborting the model chain for ${label}; this invocation is out of subrequests`,
          {
      ...
      logger.warn(`Model ${currentModel} failed for ${label}`, {
        ...
        willTryFallback:
          !outOfQuotaBudget && modelIndex < modelsToTry.length - 1,
      });
  ...
  if (sawTransientFailure) {
    ...
    const lastMessage =
      retryCause instanceof Error
        ? retryCause.message
        : String(retryCause ?? "Unknown model error");
    ...
    if (
      attemptedFailedThrough > 0 &&
      attemptedFailedThrough < wholeChain.length
    ) {
      await Promise.all(
        progressLabels.map((key) =>
          ctx.chainProgress.advance(key, attemptedFailedThrough),
        ),
      );
      Object.defineProperty(error, "nextChainIndex", {
        value: attemptedFailedThrough,
        configurable: true,
      });
    } else {
      await ctx.chainProgress.flushPending();
  ...
  if (!attemptedAnyModel) {
    ...
    if (lastError !== undefined) {
      if (isTransientModelFailure(lastError)) {
        throw new RetryableModelError(
          `Every model for ${label} failed to resolve; retrying later.`,
          lastError,
        );
      }
      throw lastError;
    ...
    throw new RetryableModelError(
      `No configured review model was attempted for ${label} (all skipped: ${
        skippedForTimeouts
          ? "repeated timeouts on this job"
          : "rate-limit cooldown or provider unavailable"
      }); retrying later.`,
    );
  }
